...
async function runAllocationAlgorithm(): Promise<[string, string][]> {
  // Fetch all data
  ...
  for (const prof of parsedProfessors) {
    ...
    limit[prof.id] = 4; // max limit by admin
  }
  ...
  // Fill student matrix with preference indices
  ...
  // Fill professor matrix with preference indices
  ...
  // Mark dropped projects as unavailable
  ...
  while (new Set(result.flat()).size !== 1) {
    ...
    // Find all positions with minimum value
    ...
    // Handle conflicts (same row = same student)
    ...
    // If same student has multiple minimum options, pick by student preference
    ...
    for (const [i, j] of nonClashIndex) {
      ...
      // Mark as unavailable if can't allocate
      else {
...
export async function GET() {
  try {
    const allocations = await runAllocationAlgorithm();
    ...
    return NextResponse.json(
      { message: "Project Allotment - successful", allocations },
      { status: 200 }
    );
  } catch (error) {
    console.error("Error in project allotment:", error);
