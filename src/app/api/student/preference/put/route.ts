...
export async function PUT(req: NextRequest) {
  try {
    ...
    const updatedPreferences = data.preference.map(
      (newPref: Preference, index: number) => {
        ...
        if (
          ...
          student.preferences.findIndex(
            (p: Preference) =>
              p.project.toString() === newPref.project.toString()
          ) !== index
        ) {
    ...
    for (const [index, pref] of updatedPreferences.entries()) {
      if (pref.isGroup && pref.partnerRollNumber) {
        ...
        const partner = await Student.findOne({
          roll_no: pref.partnerRollNumber,
        });
        if (!partner) continue;
        ...
        const partnerPrefIndex = partner.preferences.findIndex(
          (p: Preference) =>
            p.project.toString() === pref.project.toString() &&
            p.partnerRollNumber === student.roll_no
        );
        ...
        if (partnerPrefIndex === -1 || partnerPrefIndex !== index) {
          ...
          await Student.updateMany(
            {
              roll_no: { $in: [student.roll_no, partner.roll_no] },
              "preferences.project": pref.project,
            },
