...
import {
  Users, Mail, FolderKanban, CheckCircle, Clock, Search, GraduationCap, Download,
} from "lucide-react";
import * as XLSX from "xlsx";

const DOMAINS = ["ALL", "CSP", "PSPE", "VDN"];

const MtpProfessorPage = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [profRes, studRes] = await Promise.all([
          fetch(`/api/professor/get?program=MTP${domainParam}`),
          fetch(`/api/student/get?program=MTP${domainParam}`),
        ]);
        if (!profRes.ok || !studRes.ok) throw new Error("Failed to fetch data");
  ...
  const handleExportToExcel = () => {
    ...
    professors.forEach((prof) => {
      const mtpProjects = (prof.projects || []).filter((p) => p.program === "MTP");
      if (mtpProjects.length > 0) {
        mtpProjects.forEach((project) => {
          ...
          if (studentPrefs.length > 0) {
            studentPrefs.forEach((s, index) => {
              ...
              exportData.push({
                "Professor Name": prof.name, "Professor Email": prof.email,
                "Submit Status": prof.mtpSubmitStatus ? "Submitted" : "Pending",
                "Project Title": project.title, "Domain": project.domain,
                "Student Rank": index + 1, "Student Name": student.name, "Student Roll No": student.rollNo,
              });
            });
          } else {
            exportData.push({
              ...
              "Submit Status": prof.mtpSubmitStatus ? "Submitted" : "Pending",
              "Project Title": project.title, "Domain": project.domain,
              "Student Rank": "-", "Student Name": "No students ranked", "Student Roll No": "-",
            });
          }
        });
      }
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "MTP Professors");
    XLSX.writeFile(workbook, "MTP_ProfessorData.xlsx");
  };
  ...
  const stats = {
    ...
    submitted: professors.filter((p) => p.mtpSubmitStatus).length,
    pending: professors.filter((p) => !p.mtpSubmitStatus).length,
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-bold">MTP Professors</h1>
          <p className="text-muted-foreground">View professors and their MTP student preferences</p>
        </div>
        <Button onClick={handleExportToExcel} className="gap-2"><Download className="h-4 w-4" />Export to Excel</Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card><CardHeader className="flex flex-row items-center justify-between pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">Total Professors</CardTitle><Users className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold">{stats.total}</div></CardContent></Card>
      ...
      <div className="relative max-w-sm">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input placeholder="Search professors..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9" />
      </div>
      ...
      {loading ? <Loading message="Loading professors..." /> : error ? (
        ...
        <div className="space-y-4">
          {filteredProfessors.map((prof) => {
            const mtpProjects = (prof.projects || []).filter((p) => p.program === "MTP" && (activeDomain === "ALL" || p.domain === activeDomain));
            return (
              <Card key={prof.id}>
                ...
                <CardContent>
                  {mtpProjects.length > 0 ? (
                    <div className="space-y-3">
                      <div className="flex items-center gap-2 text-sm font-medium text-muted-foreground"><FolderKanban className="h-4 w-4" />MTP Projects ({mtpProjects.length})</div>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                        {mtpProjects.map((project) => {
                          ...
                          return (
                            <div key={project.id} className={`border rounded-lg p-3 transition-colors ${project.dropProject ? "bg-red-50 border-red-200" : "bg-muted/30 hover:bg-muted/50"}`}>
                              <div className="flex items-center gap-2 mb-2">
                                <h4 className="font-medium text-sm line-clamp-2 flex-1">{project.title}</h4>
                                <Badge variant="outline" className="text-[10px] h-5 shrink-0">{project.domain}</Badge>
                                {project.dropProject && <Badge variant="destructive" className="text-[10px] h-5 shrink-0">Dropped</Badge>}
                              </div>
                              {studentPrefs.length > 0 ? (
                                <div className="space-y-1">
                                  <div className="flex items-center gap-1 text-xs text-muted-foreground mb-1"><GraduationCap className="h-3 w-3" />{studentPrefs.length} student{studentPrefs.length !== 1 && "s"} ranked</div>
                                  <div className="max-h-28 overflow-y-auto space-y-1">
                                    {studentPrefs.map((s, index) => {
                                      ...
                                      return (
                                        <div key={`${s.id}-${index}`} className="flex items-center justify-between text-xs bg-background rounded px-2 py-1">
                                          <span className="truncate flex-1">{name}<span className="text-muted-foreground ml-1">({rollNo})</span></span>
                                          <Badge variant="outline" className="ml-2 text-[10px] h-5">#{index + 1}</Badge>
                                        </div>
                                      );
                                    })}
                                  </div>
...
export default MtpProfessorPage;
