...
const MtpStudentsPage = () => {
  ...
  const [projectIdTitleMap, setProjectIdTitleMap] = useState<Record<string, string>>({});
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [studRes, projRes] = await Promise.all([
          ...
          fetch(`/api/project/get?program=MTP${domainParam}`),
        ]);
        if (!studRes.ok || !projRes.ok) throw new Error("Failed to fetch data");
  ...
  const handleExportToExcel = () => {
    ...
    const formattedData = students.map((student) => {
      const row: Record<string, string | number | null | undefined> = {
        Roll_No: student.rollNo, Name: student.name, Email: student.email,
        CPI: student.cpi, Domain: student.domain,
      ...
      for (let i = 0; i < maxPrefs; i++) {
        ...
        if (pref) {
          ...
          const partner = pref.isGroup && pref.partnerRollNumber ? ` | Partner: ${pref.partnerRollNumber}` : "";
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "MTP Students");
    XLSX.writeFile(workbook, "MTP_StudentData.xlsx");
  };
  ...
  const toggleSubmitStatus = async (studentId: string, current: boolean) => {
    try {
      ...
      setStudents((prev) => prev.map((s) => s.id === studentId ? { ...s, submitStatus: !current } : s));
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-bold">MTP Students</h1>
          <p className="text-muted-foreground">Manage MTP student data and preferences</p>
        </div>
      ...
      <Tabs value={activeDomain} onValueChange={setActiveDomain}>
        <TabsList>
          {DOMAINS.map((d) => (<TabsTrigger key={d} value={d}>{d}</TabsTrigger>))}
        </TabsList>
      </Tabs>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card><CardHeader className="flex flex-row items-center justify-between pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">Total Students</CardTitle><GraduationCap className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold">{stats.total}</div></CardContent></Card>
      ...
      <div className="relative max-w-sm">
        ...
        <Input placeholder="Search by name, roll no, or email..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9" />
      </div>

      {loading ? <Loading message="Loading MTP students..." /> : error ? (
                ...
                {filteredStudents.length > 0 ? filteredStudents.map((student) => (
                  <React.Fragment key={student.id}>
                    <TableRow className="hover:bg-muted/50">
                      ...
                      <TableCell><Badge variant="outline">{student.domain}</Badge></TableCell>
                      <TableCell>
                          ...
                          <div className="space-y-1">
                            {student.preferences.slice(0, 3).map((pref, key) => (
                              <div key={pref.projectId} className="flex items-center gap-2 text-sm">
                                ...
                                {pref.isGroup ? (
                                  <Badge variant="secondary" className="text-xs gap-1"><Users className="h-3 w-3" />Group</Badge>
                                ) : (
                                  <Badge variant="outline" className="text-xs gap-1"><User className="h-3 w-3" />Solo</Badge>
                                )}
                              </div>
                            ))}
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <Button size="sm" variant={student.submitStatus ? "outline" : "default"} onClick={() => toggleSubmitStatus(student.id, student.submitStatus)}>
                            ...
                            {student.preferences.map((pref, key) => (
                              <div key={pref.projectId} className="border rounded-lg p-3 bg-background">
                                ...
                                <div className="space-y-1 text-xs text-muted-foreground">
                                  {pref.isGroup ? (
                                    <>
                                      <div className="flex items-center gap-1"><Users className="h-3 w-3" />Partner: {pref.partnerRollNumber}</div>
                                      <div className="flex items-center gap-1">Status: {pref.status === "Success" ? <Badge variant="default" className="text-[10px] h-4">Matched</Badge> : <Badge variant="secondary" className="text-[10px] h-4">Pending</Badge>}</div>
                                    </>
                                  ) : (
                                    <div className="flex items-center gap-1"><User className="h-3 w-3" />Solo project</div>
                                  )}
                                </div>
                              </div>
                            ))}
...
export default MtpStudentsPage;
