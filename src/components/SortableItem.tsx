...
export default function SortableItem({ id, project, index, isOverlay, setProjectMap, projectMap }: SortableItemProps) {
      ...
      <div className="flex justify-between w-full">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            {project.Capacity == 2 && projectMap[id].partnerRollNumber !== "" ? <>
              <p className="text-sm"> Partner's Roll Number: {projectMap[id].partnerRollNumber}<span className={`italic ${projectMap[project._id]?.status === 'Pending' ? "text-red-500" : "text-green-500"}`}>  ({projectMap[project._id]?.status})</span></p>
            </> : <></>}
          </div>
        </div>
        <div className="flex items-center gap-4">
          {project.Capacity == 2 ? (<>
            ...
            <Button onClick={handleRollNumberAdd} disabled={handleDisable()}>
              ...
              {
                projectMap[project._id]?.partnerRollNumber === "" ? "Add" : "Remove/Update"
              }
